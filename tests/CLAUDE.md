...
| `tests/core/settings_spec.lua` | settings defaults (incl. `ai_model` = glm-5.2), JSON save/load roundtrip + the `ai_model` persist path + corrupt-file fallback, the legacy background→theme migration, vim.g seeding precedence, the quiet notify filter, and the carbon accent/folder packs + single-role color slots |
...
| `tests/core/ia_spec.lua` | `:NvSinnerIA` command, the modal float rendering the settings + action rows under section headers (current model shown), the AI-completion toggle flipping + persisting `ai_complete` in place, an action row running its command + closing, the model picker's recommended-first ✓ ordering + curated fallback, `_choose_model` persisting `ai_model` (what `ai-complete.model()` returns), the NvMenuNormal surface + backdrop, and `fetch_models`' no-key/cached short-circuits |
...
| `tests/core/ai_complete_spec.lua` | inline AI completion: the pure `_build_context` (prefix/suffix split, whole-file FIM keeping imports far above the cursor, CONTEXT_RATIO budgeting when the file overflows the window) / `_build_payload`/`_extract`/`_classify` builders, the `NvAiGhost` carbon group (re-applied on ColorScheme), trigger→ghost via the swapped `_request` seam (no network), a real-grid PTY paint check (child nvim TUI in a terminal buffer: the ghost must reach the rendered screen while idle in insert mode), the in-flight loading spinner (`NvAiLoading` chip, shown on trigger / cleared on success/error / cancelled on movement), accept inserting + clearing, the model() precedence (`$OPENCODE_MODEL` > persisted `ai_model` > glm-5.2 default), and the error paths — no-key warn-once, 429 cooldown, 429 `$OPENCODE_FALLBACK_MODEL` retry, 401 auth, timeout, empty-silent — plus the insert maps (`<C-l>` trigger / `<Tab>` accept / `<C-]>` dismiss) + `:NvSinnerComplete`/`:NvSinnerCompleteToggle` |
