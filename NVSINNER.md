...
10. ✅ **Versioned releases + update check (v1.0.0, current v1.8.1).** The
    semver lives in ONE place — [lua/nvsinner/init.lua](lua/nvsinner/init.lua)
    (`version = "1.8.1"`) — and [lua/core/version.lua](lua/core/version.lua)
    ...
    `v1.8.1` plus the check status. Users update with `:NvSinnerUpdate`.
    ...
    **single click** (persisted "Neo-tree click" row) and its Buffers tab no
    ...
    which needs two diffed windows. **v1.8.1** is documentation: `docs/ARCHITECTURE.md`
    and `docs/CONTRIBUTING.md` (neither had ever existed) plus a correction pass
    over the docs that had drifted from the code — most seriously, `CLAUDE.md`
    and `docs/installation.md` both instructed `Lazy! sync`, contradicting the
    restore-not-sync non-negotiable stated in the same file, and both README and
    `CLAUDE.md` claimed `install.sh` wires `core.hooksPath`, which it never has.
