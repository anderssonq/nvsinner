...
| 8 | Headless runs never consume the health first-run marker (`setup()` bails when `#vim.api.nvim_list_uis() == 0`) | `lua/core/health.lua`; `tests/core/health_spec.lua` | Installer/tests eat the greeting; user's first launch is silent about missing or incompatible tools |
