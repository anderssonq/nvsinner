...
- `diffview.lua` — `diffview.nvim`: full side-by-side diff viewer (file panel + two
  ...
  it toggles diff ⇄ file list, never out to the buffer) and `<leader>go` (out
  to the editable buffer, closing the diff tab).
...
- The `<leader>gi`/`<leader>go` round trip has non-obvious contracts (async cursor
  ...
  `set_file_by_path` matching *relative* paths, `diffview.close(tabpage)` not
  actually closing anything, and the exit having to pre-position the target
  tab's window or `goto_file_edit` drops the file into neo-tree). Read
